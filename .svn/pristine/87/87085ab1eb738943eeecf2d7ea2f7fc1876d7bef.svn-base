<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="Modeler" stripe border :header-cell-style="{background:'#336699',color:'#eef1f6'}">
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" width="400px" append-to-body>
      <el-upload ref="upload" :limit="1" accept=".bpmn, .bar, .zip" :headers="upload.headers" :action="upload.url" :disabled="upload.isUploading" :on-progress="handleFileUploadProgress" :on-success="handleFileSuccess" :auto-upload="false" drag>
...
<script>
import { listDefinition,  delDefinition ,convert2Model,suspendOrActiveApply} from "@/api/activiti/definition";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/definition/upload",
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    convert2Model(row){
      this.$confirm('确认要转换成流程模型吗?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
        return convert2Model(row.id);
      }).then(() => {
        this.getList();
        this.msgSuccess("转换成功");
      }).catch(function() {});
